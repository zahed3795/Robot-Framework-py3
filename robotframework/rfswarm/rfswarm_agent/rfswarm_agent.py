...
class RFSwarmAgent():
    ...
    def __init__(self, master=None):
        ...
        parser.add_argument('-x', '--xmlmode', help='XML Mode, fall back to pasing the output.xml after each iteration',
                            action='store_true')
    ...
    def updatestatus(self):
        ...
        try:
            ...
            self.debugmsg(0, "Server Disconected", self.swarmserver,
                          datetime.now().isoformat(sep=' ', timespec='seconds'), "(", int(time.time()), ")")
            self.isconnected = False
        ...
        if self.swarmserver is not None:
            ...
            try:
                ...
                if (r.status_code == requests.codes.ok):
                    ...
                    self.debugmsg(0, "Server Conected", self.swarmserver,
                                  datetime.now().isoformat(sep=' ', timespec='seconds'), "(", int(time.time()), ")")
            except:
                pass
            ...
            try:
                ...
                with open(logFileName, "w") as f:
                    ...
                    try:
                        ...
                        if result != 0:
                            self.debugmsg(1, "Robot returned an error (", result, ") please check the log file:",
                                          logFileName)
                    except Exception as e:
                ...
                if self.xmlmode:
                    if os.path.exists(outputFile):
                        if self.xmlmode:
                            t = threading.Thread(target=self.run_process_output, args=(
                            outputFile, self.jobs[jobid]["ScriptIndex"], self.jobs[jobid]["VUser"],
                            self.jobs[jobid]["Iteration"]))
                            t.start()
                    else:
                        self.debugmsg(1, "Robot didn't create (", outputFile, ") please check the log file:",
                                      logFileName)
            except Exception as e:
